...
import com.symbol.kumkangpop.model.ProgressFloorReturnService;
...
public class ProgressFloorReturnViewModel extends ViewModel {

    // 사용자에게 보여줄 국가 데이터
    public MutableLiveData<List<Dong>> list = new MutableLiveData<>();
    // 데이터를 가져오는 것에 성공했는지를 알려주는 데이터
    public MutableLiveData<Boolean> loadError = new MutableLiveData<>();
    // 로딩 중인지를 나타내는 데이터
    public MutableLiveData<Boolean> loading = new MutableLiveData<>();
    // 서버에서 발생한 에러의 내용
    //public MutableLiveData<String> errorMsg = new MutableLiveData<>();
    public ProgressFloorReturnService service = ProgressFloorReturnService.getInstance();
    ...
    public void refresh(SearchCondition searchCondition) {
        getData(searchCondition);
    }

    private void getData(SearchCondition searchCondition) {
        ...
        disposable.add(
                ...
                service.getData(searchCondition) // Single<List<T>>를 반환한다.
                ...
                .subscribeWith(new DisposableSingleObserver<List<Dong>>() {
                    @Override
                    public void onSuccess(@NonNull List<Dong> models) {
